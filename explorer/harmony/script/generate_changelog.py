...
from build import DEFAULT_MODULES, PLATFORM_HARMONY_DIR, collect_module_config_list

def generate_changelog(module_path, version):
    ...
    cmd = f"git-cliff --config {changelog_template_path} --latest | sed 's/<!-- version -->/{version}/g' > {module_path}/CHANGELOG.md"
    print(f'run command {cmd}')
    check_call(cmd, shell=True, cwd=PLATFORM_HARMONY_DIR)
...
def main(argv):
    ...
    for module in modules:
        ...
        generate_changelog(module_full_path, args.version)
